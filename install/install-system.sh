...
EOF
    ...
    if [ _"$WODREVPROXY" != _"0" ]; then
        sudo certbot --nginx -d $WODAPIDBEXTFQDN,$WODFEEXTFQDN,$WODBEEXTFQDN -n --https-port $WODAPIDBEXTPORT --agree-tos -m $WODREVPROXY
    fi
elif [ $WODTYPE = "frontend" ]; then
    cd $WODFEDIR
    mkdir -p .cache
